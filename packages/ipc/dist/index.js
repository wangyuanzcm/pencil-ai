...
var WebSocketRequestRouter = class {
  ...
  findDevice(filePath) {
    ...
    for (const [resourcePath, device] of this.deviceMap) {
      if (resourcePath === filePath || path.resolve(resourcePath) === resolvedPath) {
        return device;
      }
    }
    return void 0;
...
var IPCDeviceManager = class {
  ...
  addResource(ipc, device) {
    ...
    ipc.handle("find-libraries", async () => (await device.findLibraries()).map((path3) => pathToFileURL(path3).toString()));
    ...
    ipc.handle(
      "browse-libraries",
      async ({ multiple }) => (await device.browseLibraries(multiple))?.map((path3) => pathToFileURL(path3).toString())
    );
  }
  async stopAllAgents() {
