...
class IPCHost {
    ...
    async handleRequest(message) {
        ...
        try {
            ...
            const ipcError = error instanceof ipc_types_1.IPCError
                ? error
                : new ipc_types_1.IPCError("HANDLER_ERROR", error instanceof Error ? error.message : "Unknown error", error instanceof Error ? error.stack : "Unknown stack trace");
            this.sendResponse(message.id, message.method, undefined, {
                code: ipcError.code,
    ...
    handleResponse(message) {
        ...
        if (message.error) {
            const error = new ipc_types_1.IPCError(message.error.code, message.error.message, message.error.stack);
        ...
        return new Promise((resolve, reject) => {
            ...
            const timeout = timeoutMs >= 0
                ? setTimeout(() => {
                    ...
                    reject(new ipc_types_1.IPCError("TIMEOUT", `Request '${method}' timed out after ${timeoutMs}ms`));
                }, timeoutMs)
                : undefined;
    ...
    dispose() {
        this.pendingRequests.forEach(({ reject, timeout }) => {
            ...
            reject(new ipc_types_1.IPCError("DISPOSED", "IPC server was disposed"));
        });
