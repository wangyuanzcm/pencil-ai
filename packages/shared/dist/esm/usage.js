...
export function setBillingEnabled(enabled) {
    _billingEnabled = enabled;
}
export function isBillingEnabled() {
    return _billingEnabled;
}
const BILLING_DISABLED_CHECK = { allowed: true, isMetered: false };
const BILLING_DISABLED_RECORD = { success: true, newUsed: 0 };
/**
 ...
 */
export async function checkFeature(organizationId, featureKey, apiBase) {
    if (!_billingEnabled)
        return BILLING_DISABLED_CHECK;
    try {
 ...
 */
export async function recordUsage(organizationId, featureKey, apiBase, count = 1) {
    if (!_billingEnabled)
        return BILLING_DISABLED_RECORD;
    try {
 ...
 */
export async function checkAndRecordUsage(organizationId, featureKey, apiBase) {
    if (!_billingEnabled)
        return BILLING_DISABLED_CHECK;
